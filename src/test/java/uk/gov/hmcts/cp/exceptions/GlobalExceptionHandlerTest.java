...
class GlobalExceptionHandlerTest {
    ...
    private static final Instant FIXED_INSTANT = Instant.parse("2026-06-24T10:00:00Z");

    @Spy
    Tracer tracer = Tracer.NOOP;

    @Spy
    ClockService clockService = new ClockService(Clock.fixed(FIXED_INSTANT, ZoneOffset.UTC));
    ...
    private void assertErrorFields(ResponseEntity<ErrorResponse> errorResponse, HttpStatusCode httpStatusCode, String message) {
        ...
        assertThat(errorResponse.getBody().getTimestamp()).isEqualTo(FIXED_INSTANT);
    }
}